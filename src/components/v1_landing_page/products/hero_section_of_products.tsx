...
// Info: (240705 - Liz) 定義投影片的類型
interface Slide {
  id: number;
  content: SlideContent;
}

// Info: (240705 - Liz) 定義投影片的內容
const Slide1 = () => <KvVideo />;

const Slide1Updated = () => <SliderCover />;

const Slide2 = () => (
  <section className="relative min-h-875px py-80px">
...
const HeroSectionOfProducts: React.FC = () => {
  ...
  useEffect(() => {
    if (hasCompletedFirstCycle) {
      setSlides((prevSlides) => [
        { ...prevSlides[0], content: <Slide1Updated /> },
        ...prevSlides.slice(1),
      ]);
    }
  ...
  const settings = {
    ...
    beforeChange: (currentSlide: number, nextSlide: number) => {
      if (currentSlide === 3 && nextSlide === 0 && !hasCompletedFirstCycle) {
